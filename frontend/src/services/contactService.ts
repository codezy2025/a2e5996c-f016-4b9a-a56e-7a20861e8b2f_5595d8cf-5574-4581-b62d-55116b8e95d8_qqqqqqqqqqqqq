/**
 * 🏗️  DEVELOPMENT GUIDE - Contact Service
 * 
...
import { Contact, CreateContactDto, UpdateContactDto } from '../types/ContactTypes';

const API_BASE_URL = 'http://localhost:3000/api/contacts';

const contactService = {
  getAll: async (): Promise<Contact[]> => {
    const response = await axios.get<Contact[]>(API_BASE_URL);
    return response.data;
  },

  create: async (contactData: CreateContactDto): Promise<Contact> => {
    const response = await axios.post<Contact>(API_BASE_URL, contactData);
    return response.data;
  },

  update: async (id: string, contactData: UpdateContactDto): Promise<Contact> => {
    const response = await axios.put<Contact>(`${API_BASE_URL}/${id}`, contactData);
    return response.data;
  },
