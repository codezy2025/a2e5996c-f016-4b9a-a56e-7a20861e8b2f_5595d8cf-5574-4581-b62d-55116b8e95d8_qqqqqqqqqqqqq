/**
 * 🏗️  DEVELOPMENT GUIDE - Home Service
 * 
 ...
 * - search(query: string): Promise<Home[]>
...
import { Home, HomeCreateRequest, HomeUpdateRequest } from '../types/HomeTypes';

const API_BASE_URL = 'http://localhost:3000/api/homes';

export const homeService = {
    getAll: async (): Promise<Home[]> => {
        const response = await axios.get<Home[]>(API_BASE_URL);
        return response.data;
    },
    create: async (homeData: HomeCreateRequest): Promise<Home> => {
        const response = await axios.post<Home>(API_BASE_URL, homeData);
        return response.data;
    },
    update: async (id: string, homeData: HomeUpdateRequest): Promise<Home> => {
        const response = await axios.put<Home>(`${API_BASE_URL}/${id}`, homeData);
        return response.data;
    },
